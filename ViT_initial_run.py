...
imageClassifier = ImageClassifierViT().to('mps')
...
print("\n\n--- Testing with PEFT and projection ---\n")
...
for input, target in tqdm(cifar10_train_loader, ncols=50):
    ...
    input = imageClassifier.backbone.processor( #type: ignore
        images=input,
        return_tensors="pt"
    )["pixel_values"].to('mps')
    optimizer.zero_grad()
...
Q, Lambda = compute_Q_for_task(
    imageClassifier,
    cifar10_train_loader,
    device='mps',
    k=16,
    processor=imageClassifier.backbone.processor,
    use_processor=True
)
imageClassifier.set_active_task('CIFAR100')
imageClassifier.set_Q(Q, None)

print("Finished setting Q for CIFAR100 after CIFAR10.")

optimizer = imageClassifier.configure_optimizers()

for input, target in tqdm(cifar100_train_loader, ncols=50):
    input, target = input.to('mps'), target.to('mps')
    input = imageClassifier.preprocess_inputs(input)
    optimizer.zero_grad()
    output = imageClassifier(input)
    loss = imageClassifier.criterion(output, target)
    loss.backward()
...
imageClassifierWithoutPeft = ImageClassifierViT().to('mps')
